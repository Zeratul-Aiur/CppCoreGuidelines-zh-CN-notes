...
- 与使用`make_shared`来替代的情况类似，许多的子表达式（包括函数参数）的求值顺序都是未指明的。一旦其中一个构造函数调用抛出了异常，那么另一个对象的内存可能永远不会被释放了。

## ES.7: 保持常用的和局部的名字尽量简短，而让非常用的和非局部的名字较长

- 可读性。降低在无关的非局部名字之间发生冲突的机会。

## ES.63: 不要产生切片

- 切片：将一个派生类对象赋值给基类对象时，派生类对象中派生类特有的部分会被丢失。
- 对象总是被当成一个整体。进行的蓄意的切片，其代码会让人意外，更好的做法是，提供一个明确的操作。
...
- 可读性。
  ...
  ```cpp
  int events = 0;
  for (; wait_for_event(); ++events) { // events 跟 wait_for_event() 没有任何关系
    // ...
  }
...
- 性能对于 Cache 非常敏感，而 Cache 则更喜欢对相邻数据进行（通常是线性的）简单访问。
